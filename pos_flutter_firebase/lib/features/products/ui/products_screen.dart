...
import '../../../shared/widgets/product_presentation.dart';
  ...
  Widget? _buildFab() {
    if (_tabController.index == 0) {
      return FloatingActionButton(
        ...
        onPressed: () {
          context.push('/home/products/add', extra: {
    ...
    return StreamBuilder<List<Product>>(
      ...
      builder: (context, snapshot) {
        ...
        return StreamBuilder<Map<String, ProductStock>>(
          stream: context.read<StockRepository>().watchStockByStore(businessId: businessId, storeId: storeId),
                ...
                children: [
                  ...
                  PopupMenuButton<String>(
                    ...
                    onSelected: (value) async {
                      if (value == 'edit') {
                        await context.push('/home/products/edit', extra: {
...
Future<void> _confirmDeactivateProduct(
  ...
  try {
    await context.read<ProductRepository>().deactivateProduct(businessId: businessId, product: product);
...
class _CategoriesTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<app_models.Category>>(
      stream: context.read<CategoryRepository>().watchCategories(businessId: businessId),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const Center(child: CircularProgressIndicator());
        }
...
Future<void> _confirmDeactivateCategory(
  ...
  try {
    await context.read<CategoryRepository>().deactivateCategory(businessId: businessId, categoryId: category.id);
    if (!context.mounted) return;
...
class _ModifiersTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<Modifier>>(
      stream: context.read<ModifierRepository>().watchModifiers(businessId: businessId),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const Center(child: CircularProgressIndicator());
        }
...
Future<void> _confirmDeactivateModifier(
  ...
  try {
    await context.read<ModifierRepository>().deactivateModifier(businessId: businessId, modifierId: modifier.id);
    if (!context.mounted) return;
...
class _DiscountsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<Discount>>(
      stream: context.read<DiscountRepository>().watchDiscounts(businessId: businessId),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const Center(child: CircularProgressIndicator());
        }
...
Future<void> _confirmDeactivateDiscount(
  ...
  try {
    await context.read<DiscountRepository>().deactivateDiscount(businessId: businessId, discountId: discount.id);
    if (!context.mounted) return;
