...
class _ReportsTabState extends State<_ReportsTab> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<Sale>>(
      stream: SaleService().watchBusinessSales(businessId: widget.businessId),
      builder: (context, salesSnapshot) {
        ...
        return StreamBuilder<List<Employee>>(
          stream: EmployeeService().watchEmployees(businessId: widget.businessId),
  ...
  Widget _cortesDeCajaCard(BuildContext context) {
    return StreamBuilder<List<Shift>>(
      stream: ShiftService().watchAllClosedShifts(businessId: widget.businessId),
      ...
      builder: (context, productsSnapshot) {
        ...
        return StreamBuilder<Map<String, ProductStock>>(
          stream: selectedStoreId == null
              ...
              : StockService().watchStockByStore(businessId: businessId, storeId: selectedStoreId!),
          ...
          children: [
            ...
            StreamBuilder<List<InventoryMovement>>(
              stream: InventoryService().watchMovements(businessId: businessId),
              builder: (context, snapshot) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<List<Employee>>(
        stream: EmployeeService().watchEmployees(businessId: businessId),
        builder: (context, snapshot) {
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                ...
                onTap: employee.isOwner ? null : () => _showEmployeeDialog(context, employee: employee),
              );
            },
          );
        },
...
class _EmployeeDialogState extends State<_EmployeeDialog> {
  ...
  void initState() {
    ...
    _pinController.text = employee?.pin ?? '';
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              obscureText: true,
              decoration: const InputDecoration(labelText: 'PIN', helperText: 'Minimo 4 digitos', border: OutlineInputBorder()),
            ),
            const SizedBox(height: 12),
  ...
  void _submit() {
    ...
    if (name.isEmpty || email.isEmpty || pin.length < 4 || _storeIds.isEmpty) {
      setState(() => _errorMessage = 'Completa nombre, correo, PIN y sucursal');
      return;
    }
